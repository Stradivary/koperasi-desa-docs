# Tech Specs Index

This folder contains implementation-focused technical specifications for the NFC wallet application. These sections are intended for developers building the frontend, backend, and card interfaces.

## Sections

1. [Overview](1_overview.md)
2. [System Architecture](2_system-architecture.md)
3. [Card Storage Model](3_card-storage-model.md)
4. [Cryptography](4_cryptography.md)
5. [Tamper Detection & Validation](5_tamper-detection-validation.md)
6. [State Machine & Session Rules](6_state-machine-session-rules.md)
7. [Write & Update Strategy](7_write-update-strategy.md)
8. [Backend & Frontend Interfaces](8_backend-frontend-interfaces.md)
9. [Risk & Financial Limits](9_risk-financial-limits.md)
10. [Implementation Notes](10_implementation-notes.md)
11. [Deployment & Maintenance](11_deployment-maintenance.md)
12. [Key Hierarchy & Session Grants](12_key-hierarchy-session-grants.md)
13. [Role-specific App Models](13_roles-app-models.md)
14. [Transaction Log Format](14_transaction-log-format.md)
15. [Status Codes & Block Rules](15_status-codes-block-rules.md)
16. [Infrastructure Stack](16_infrastructure-stack.md)
17. [Time, Validation & Assumptions](17_time-validation-assumptions.md)

> These specs are written as a development reference: clear behavior definitions, interface expectations, and implementation guidance for both the browser app and the backend service.
